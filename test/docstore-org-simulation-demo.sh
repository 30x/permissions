...
EOF)
command='echo $permissions | envsubst | curl http://localhost:8080/az-permissions -d @-  -H "Content-Type: application/json" -H "Authorization: Bearer $APIGEE_TOKEN3" -D - -o ttx.txt -sS' 
while IFS=':' read key value; do
    value=${value##+([[:space:]])}; value=${value%%+([[:space:]])}
    if [ "$key" == 'Etag' ]; then
        ACME_ORG_FOLDER_PERMISSIONS_ETAG="$value";
...
command='curl "http://localhost:8080/az-is-allowed?resource=http://apigee.com/spec/12345&user=$APIGEE_USER3&action=read" -H "Accept: application/json" -H "Authorization: Bearer $APIGEE_TOKEN3" -Ss'
echo $command
echo -n "Have APIGEE_USER1 ask if APIGEE_USER1 can read http://apigee.com/spec/12345: "
echo -e "\x1B[7m$(eval $command)\x1B[27m" 
echo ''
command='curl "http://localhost:8080/az-is-allowed?resource=http://apigee.com/spec/12345&user=$APIGEE_USER3&action=update" -H "Accept: application/json" -H "Authorization: Bearer $APIGEE_TOKEN3" -Ss'
echo $command
...
EOF)
command='echo $patch | envsubst | curl http://localhost:8080/az-permissions?http://apigee.com/folder/12345 -d @- -X PATCH -H "Content-Type: application/merge-patch+json" -H "Authorization: Bearer $APIGEE_TOKEN1" -H "If-Match: $ACME_ORG_FOLDER_PERMISSIONS_ETAG" -D - -o ttx.txt -sS'
while IFS=':' read key value; do
    value=${value##+([[:space:]])}; value=${value%%+([[:space:]])}
    if [ "$key" == 'Etag' ]; then
        ACME_ORG_FOLDER_PERMISSIONS_ETAG="$value";
